...
int vcb_sg_stat_code[10] = { 0x00, //0:dark
		0x02, //1:amber
		0x01, //2:min-red
		0x01, //3:ext-red
		0x02, //4:prep
...
//debug(1, "det:%d, value:%d\n", det, value);
	...
	if (g_tv_det.tv_sec != tv.tv_sec) {
		if (g_tv_det.tv_sec != 0) {		//保存文件
			...
			sprintf(str, "log/%d_%04d%02d%02d%02d%02d%02d_det.ini", vcb_FNr,
					gst.tm_year + 1900, gst.tm_mon + 1, gst.tm_mday,
					gst.tm_hour, gst.tm_min, gst.tm_sec);
	...
	sprintf(str + strlen(str), "Time=%04d%02d%02d%02d%02d%02d%03ld\r\n",
			...
			st.tm_sec, (tv.tv_usec / 100000) * 100);
	//printf("%s", str);
...
int drv_add_sg(int sg, int stat)
{
	...
	if (g_tv_sg.tv_sec != tv.tv_sec) {
		if (g_tv_sg.tv_sec != 0) {	//保存文件
			...
			FILE* fp = NULL;
			memset(str, 0, sizeof(str));
			sprintf(str, "log/%d_%04d%02d%02d%02d%02d%02d_sg.ini", vcb_FNr,
					gst.tm_year + 1900, gst.tm_mon + 1, gst.tm_mday,
					gst.tm_hour, gst.tm_min, gst.tm_sec);
			fp = fopen(str, "wb");
			if (fp == NULL) {
...
int drv_add_ap(void)
{
	...
	if (g_tv_ap.tv_sec != tv.tv_sec) {
		if (g_tv_ap.tv_sec != 0) {		//首次无数据
			...
			if (size) {
				memset(str, 0, sizeof(str));
				sprintf(str, "log/%d_%04d%02d%02d%02d%02d%02d_ap.ini", vcb_FNr,
						gst.tm_year + 1900, gst.tm_mon + 1, gst.tm_mday,
						gst.tm_hour, gst.tm_min, gst.tm_sec);
				FILE* fp = fopen(str, "wb");
				if (fp == NULL) {
				...
				fwrite((unsigned char*) g_ptr_ap_diff, 1,
						strlen((unsigned char*) g_ptr_ap_diff), fp);
				fclose(fp);
#ifdef LOGBAK
				memset(str, 0, sizeof(str));
				sprintf(str,
	...
	for (i = 0; i < g_ap_def_inst; i++) {
		...
		if (py != g_ptr_ap_def[i].val) {		//检查ap值并打印ini记录
			...
			sprintf(str, "[%d_%ld.%ld_%d %d]\r\n", vcb_FNr,
					(px.id >> 16) & 0xffff, px.id & 0xffff, px.inst, diff);
			sprintf(str + strlen(str), "Value=%d\r\n", py);
			sprintf(str + strlen(str), "Time=%04d%02d%02d%02d%02d%02d%03ld\r\n",
					st.tm_year + 1900, st.tm_mon + 1, st.tm_mday, st.tm_hour,
					st.tm_min, st.tm_sec, (tv.tv_usec / 100000) * 100);
